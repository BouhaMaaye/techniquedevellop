...
void affiche_ligne (int c, int* ligne){
	...
	for(i=0; i<c; ++i){ 
	  ...
	  if(ligne[i] > 0 && ligne[i] < 10){
	    printf("| %d ", ligne[i]); //un espace à gauche et à droite
	  }
	  else if(ligne[i] >= 10 && ligne[i] < 100){
	    printf("| %d", ligne[i]);  //un espace à gauche
	  }
	  else if(ligne[i] >= 100){
	    printf("|%d", ligne[i]);  //sans espace
	  }
	}
	printf("|\n");
	return;
